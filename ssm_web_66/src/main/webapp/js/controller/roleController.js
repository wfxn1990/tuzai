...
app.controller('roleController' ,function($scope,$controller,roleService){
    ...
    $scope.findById = function(id){
        roleService.findOne(id).success(function(data){
	...
	$scope.del=function(){
		//获取选中的复选框			
        roleService.del( $scope.selectIds ).success(
			function(response){
				if(response.flag){
	...
	$scope.search=function(page,rows){			
		roleService.search(page,rows).success(
			function(response){
	...
	$scope.updateStatus = function(sellerId,status){
        roleService.updateStatus(sellerId,status).success(function(response){
			if(response.flag){
				//重新查询 
	        	$scope.reloadList();//重新加载
			}else{
